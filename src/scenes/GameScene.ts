...
export class GameScene implements Scene {
    ...
    constructor(game: Game) {
        ...
        this.map = new MapManager(this.game.canvas.width, this.game.canvas.height);
        this.effects = new EffectSystem(this.game.ctx);
        this.debug = new DebugSystem(this);
        
        this.cardSys = new CardSystem(this); // Создаем cardSys раньше forge, так как forge может зависеть от него (логически)
    ...
    public start() {
        ...
        this.debug.log("Game Scene Started");
    }
    ...
    public restart() {
        ...
        // --- ИСПРАВЛЕНИЕ ЗДЕСЬ ---
        // Было: this.forge.slots = [null, null];
        // Стало:
    ...
    public handleGridClick(col: number, row: number) {
        ...
        this.selectedTower = clickedTower || null;
    }
    ...
    public update() {
        ...
        this.forge.update(); // Добавил вызов update для forge, если он нужен
        ...
        for (let i = this.enemies.length - 1; i >= 0; i--) {
            ...
            if (!e.isAlive()) {
                ...
                // Visual Debris
                for(let k=0; k<6; k++) {
                    const conf = (CONFIG.ENEMY_TYPES as any)[e.typeId.toUpperCase()]; 
                    this.effects.add({
                        type: 'debris', x: e.x, y: e.y,
                        vx: (Math.random() - 0.5) * 6, vy: (Math.random() - 0.5) * 6,
                        life: 40, color: conf ? conf.color : '#fff', size: 6,
    ...
    public draw(ctx: CanvasRenderingContext2D) {
        ...
        if (targetTower && !targetTower.isBuilding) {
            ...
            ctx.beginPath();
            ctx.arc(targetTower.x, targetTower.y, stats.range, 0, Math.PI * 2);
            ctx.lineWidth = 1;
        ...
        this.enemies.forEach(e => {
            ...
            ctx.fillStyle = '#000'; ctx.fillRect(barX - 1, barY - 1, barWidth + 2, barHeight + 2);
