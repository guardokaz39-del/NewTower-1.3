...
### 9. Text Rendering & UI LOD [NEW]

- **Запрещено**: `ctx.strokeText` или `fillText` для обычного массового текста (урон каждого выстрела, иконки статусов дебаффов "Burn", "Slow" над каждым мобом). Это очень дорого.
- **Разрешено**: Текст-эффекты только для редких событий (Critical Hit, Level Up).
- **Обязательно (LOD UI)**: Для массовых статус-эффектов используйте микро-примитивы (например, `ctx.fillRect(x, y, 3, 3)` красного цвета у HP-бара для обозначения горения). Микро-примитивы батчатся движком браузера и рисуются почти бесплатно, в отличие от векторных шрифтов.
